...
from compute_last_round_key import get_last_key
...
def write_files_tv(cases,fn_in,fn_out,inverse):
    ...
    for ci,c in enumerate(cases):
        ...
        if not inverse:
            ...
            write_data(fin,"plaintext",bytes2hexstr(c))
            write_data(fin,"umsk_key",str(get_last_key(int.from_bytes(k, "big"))))
        write_data(fin,"last",str(last[ci]))
        # Write data OUT
        if not inverse:
            write_data(fout,"ciphertext",bytes2hexstr(c))
        else:
            write_data(fout,"ciphertext",bytes2hexstr(p))
...
if __name__ == "__main__":
    ...
    ## Write files    
    write_files_tv(cases,fn_in,fn_out,args.inverse)
